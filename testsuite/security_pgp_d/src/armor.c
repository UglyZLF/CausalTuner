 ...
 */
static int
getlines(char *buf, int n, FILE * f)
{
    int state;
    char *p;
    int c;
...
"armor_file: infile = %s, outfile = %s, filename = %s, clearname = %s\n",
	...
	while ((i = getlines(buffer, sizeof buffer, clearFile)) >= 0) {
...
LANG("\n\007Unable to write ciphertext output file '%s'.\n"), litfile);
	...
	for (;;) {
	    ++infile_line;
	    nline = status;
	    status = getlines(buf, sizeof buf, in);
	    if (status < 0) {
		fprintf(pgpout,
LANG("ERROR: ASCII armor decode input ended unexpectedly!\n"));
		fclose(in);
	    ...
	    /* Ignore error; getlines will discover it again */
	}
	fflush(litout);
	if (ferror(litout)) {
	    fclose(litout);

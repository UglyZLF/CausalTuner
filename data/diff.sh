...
do
	...
	gcc -Q -O0 --help=$item >> /tmp/1
	gcc -Q -O3 --help=$item >> /tmp/2
done

diff /tmp/1 /tmp/2 > /tmp/diff

cat /tmp/diff | grep "enabled" | awk '{print $2}' | sort | uniq | xargs echo -n 
# cat /tmp/diff | grep "启用" | awk '{print $2'} | sort | uniq | xargs echo -n 
rm /tmp/1 /tmp/2 /tmp/diff
